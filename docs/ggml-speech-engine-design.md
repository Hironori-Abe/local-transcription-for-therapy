...
- 作成日: 2026-09-25
- ブランチ: `N3-Diarization`
- 状態: **設計段階（未実装）**。PoC の手順と実測値は `demo_data/ggml-poc/README.md` を参照
...
| `beam_size=3, best_of=3`（低メモリモードでは1/1） | `-bs 3 -bo 3`（低メモリモードでは `-bs 1 -bo 1`） | whisper-cli の既定値は 5/5 |
| `condition_on_previous_text=False` | `-mc 0` | whisper.cpp は既定で直前のテキストを引き継ぐ。雪崩型ハルシネーション防止の方針に合わせて無効化する |
| `initial_prompt` | **初期実装では渡さない** | 5.2 参照 |
...
そのため初期実装ではプロンプトを渡さない。用語辞書を活かす方法は別タスクで検証する。候補は、223トークン以内に収めたプロンプトを `--carry-initial-prompt` で毎回の窓に付ける方法だが、PoC ではセグメントが長くなりすぎる副作用が出ている。
...
pyannote 経路は exclusive 出力（重なりの無い区間）を使っている。Nemotron は重なりを含む区間を返す（PoC では10分中91秒）。文字起こしの行への割り当ては重なり最大で決まるため、そのままでも破綻はしない。ただし `summary.speakers[].duration` の意味が変わる（重なり分を二重に数える）ため、区間を exclusive に変換してから集計する。
