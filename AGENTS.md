...
- **必要アセット**: E4B 本体 GGUF は校正用と同一ファイルを共有（追加DL不要）。新規に必要なのは `mmproj-BF16.gguf`（約992MB、`unsloth/gemma-4-E4B-it-qat-GGUF`、`clip.audio.projector_type=gemma4a`）のみで、設定タブ「音声入力パック」から**後付けDL**（E4B と同じモデルディレクトリへ配置）。Full 版のパック導入判定は本体+mmproj のみ（`cpu_backend_required=false`）。Editor 版は従来どおり CPU バックエンド込み。
