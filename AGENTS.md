...
- LLM proofreading: Gemma 4 E4B + Lemonade / llama.cpp / local OpenAI-compatible API（loopback only）
...
- MTP ドラフト（`mtp-gemma-4-E4B-it.gguf`、約60MB）は setup スクリプトが Gemma 本体と一緒に**無条件でダウンロード**する（CUDA/AMD 問わず）。「ダウンロードした記憶がないファイル」はこれで、正常
- 実際に MTP を使うのは **NVIDIA の同梱 llama-server 直起動経路のみ**（`try_start_llama_server_cuda`）。`--spec-type draft-mtp` / `--spec-draft-model` を渡す。MTP 使用時は既知バグ回避のため FlashAttention を off にする（llama.cpp #24376 / #24457、Unsloth MTP README）
...
- 既定の Gemma 4 E4B / Lemonade 校正経路。特にデフォルトプロンプトと実行条件は、互換API追加の影響を受けないように保つ
