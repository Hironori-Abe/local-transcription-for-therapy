...
3. **OS・WebView・ドライバ自体の通信**: Windows・画面表示に使う WebView2・グラフィックドライバなどのシステム構成要素は、本アプリと無関係に必須診断や更新確認等の通信を行うことがあります。本アプリは WebView2 のクラッシュダンプ自動送信を無効化していますが、システム側の通信すべてを制御することはできません。組織として完全なオフライン運用を求める場合は、OS やファイアウォール側の設定を併用してください。
