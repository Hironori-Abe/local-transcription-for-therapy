...
- 実際に MTP を使うのは **NVIDIA の同梱 llama-server 直起動経路のみ**（`try_start_llama_server_cuda`）。E4B・12B の両階層で同じ配線を使い、`--spec-type draft-mtp` / `--spec-draft-model` / `--spec-draft-n-max 3` / `--spec-draft-ngl 99` を渡す。FlashAttention は MTP 併用時も含めて **on**（12B QAT + MTP + `--flash-attn on` の実機通過を確認済み）
