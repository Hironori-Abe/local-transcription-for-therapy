...
| Lemonade embeddable zip | `src-tauri/resources/lemonade/` | `LEMONADE_VERSION`（bat 内） |
...
### `tauri.build.nvidia-windows.override.json` が必須な理由

`tauri.conf.json`（開発用）には `.venv312` や話者分離モデルが `resources` に含まれています。NSIS ビルドでこれらを同梱すると数 GB 超のインストーラーになります。`tauri.build.nvidia-windows.override.json` は `resources` リストを上書きして venv・モデルを除外し、代わりに embedded Python runtime（`resources/python312/`）、LGPL ffmpeg（`resources/ffmpeg/`）、セットアップスクリプト群、`LICENSE` / `NOTICE` / `THIRD_PARTY_LICENSES.md` / `licenses/` を含めます。
...
`setup-build-tools.bat` は `--config tauri.build.nvidia-windows.override.json` を自動で指定するため、**手動で `cargo tauri build` を実行する場合も必ずこのオプションを付ける**こと。
...
`src-tauri/nsis/lemonade-hooks.nsh` は Tauri の NSIS インストーラーフックで、インストール / アンインストール時に Lemonade プロセスの起動・停止を管理します。
...
```json
"nsis": {
  "installerHooks": "nsis/lemonade-hooks.nsh",
  "languages": ["Japanese"],
...
- [ ] `scripts/setup-build-tools.bat` の `LEMONADE_VERSION` / `PYTHON_VERSION` が最新か確認
...
| override | 用途 |
| --- | --- |
| `tauri.build.nvidia-windows.override.json` | 安定版 / NVIDIA RTX 主軸 / Windows NSIS（`setup-build-tools.bat` が使用） |
| `tauri.build.nvidia-ubuntu.override.json` | NVIDIA / Ubuntu ビルド用（詳細調整予定） |
| `tauri.build.amd-windows.override.json` | AMD / Windows NSIS ビルド用（詳細調整予定） |
| `tauri.build.amd-ubuntu.override.json` | AMD experimental / ROCm, HIPBLAS, Lemonade 検証用 / Ubuntu |
| `tauri.editor.windows.override.json` | 軽量 Editor 版 / Windows NSIS（Lemonade 非搭載のため `nsis/editor-hooks.nsh` を使用） |
| `tauri.editor.ubuntu.override.json` | 軽量 Editor 版 / Ubuntu deb |
...
```sh
# Ubuntu (deb)
cargo tauri build --bundles deb --config tauri.editor.ubuntu.override.json
```
