...
AMD 用 llama.cpp `llama-server`（ROCm / Vulkan）とEditor版・CPU版のCPUビルドは、本体アプリとは別に**上流リリースから直接ダウンロード**する（`download_llama_backend_cli.py`、`ggml-org/llama.cpp` の Releases）。AMDは `DEFAULT_BUILD = b9631`、CPUは `CPU_DEFAULT_BUILD = b10075`。lemond デーモン・lemonade CLI は撤去済みで、取得にも配信にも使わない。NVIDIA の CUDA llama-serverも公式 Windows CUDA 12.4 **b10075** をインストーラー同梱する（`resources/llama-server/`）。CPUとNVIDIAはb10075、AMDのROCm / Vulkanはb9631として更新系統を分離する。
