...
MAX_BATCH_SEGMENTS_LEMONADE = 40  # Lemonade用バッチサイズ
...
# lemonade(CUDA/lemond) 経路のみ。openai 互換は外部サーバー負荷を避け 1 固定。
...
def build_chat_messages(
    ...
    """Lemonade chat completions API 用のメッセージリストを生成する。"""
    context_lines = []
    if prev_context:
        context_lines.append(f"前の文（参考）：{prev_context}")
    if next_context:
...
def _stream_llm_chat(
    ...
    lemond.exe が stream=true をサポートしない場合も通常レスポンスとして動作する。"""
...
def _proofread_segments_openai_chat(
    ...
    def _fetch_models_data():
        ...
        while True:
            attempt += 1
            try:
                # lemond.exe 起動直後はモデル列挙に時間がかかるため timeout を長めに設定
...
def proofread_segments_llm(
    ...
    # llama-server は thinking=1 がデフォルト。Lemonade はサーバー設定で無効化済みだが
    # このパラメータを渡しても無害。
