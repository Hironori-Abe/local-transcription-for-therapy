...
設定タブ「校正用AIモデル」セクションの「校正AIモデル」セレクタで、内蔵校正モデルの階層を選べる。

| 階層 | モデル | 既定 | 対象 | 取得方法 |
| --- | --- | --- | --- | --- |
| 標準 | Gemma 4 E4B QAT（+MTP） | ✅ | CUDA / AMD 共通 | setup スクリプトで同梱取得（従来どおり） |
| 高精度 | Gemma 4 12B QAT + MTP | | **NVIDIA / CUDA 版のみ** | large-v3 と同じく**後からダウンロード**（約7GB） |

- **既定は E4B（標準）**。12B は「上位モデル」としてのオプトインで、選択しなければ従来どおり E4B 経路（デフォルトプロンプト・実行条件とも不変）。
- 12B は `unsloth/gemma-4-12B-it-qat-GGUF`（本体 `gemma-4-12B-it-qat-UD-Q4_K_XL.gguf` + ドラフト `mtp-gemma-4-12B-it.gguf`）を `download_gemma_gguf_cli.py --model 12b` で取得する。配置先は E4B と並ぶ `python_sidecar/models/llm/gemma-4-12b-it/`（リリースは `app_local_data_dir()/models/llm/gemma-4-12b-it/`）。
- **AMD 版は 12B 非対応**（AMD/Lemonade 経路に MTP が未配線のため）。`read_proofread_model_tier` / `set_proofread_model_tier` / `download_gemma_12b` のいずれも AMD 版では常に E4b に丸める。フロントのセレクタも CUDA 版でのみ表示する。
- 選択の単一の真実は `app_local_data_dir()/proofread-model-tier.txt`（内容 `e4b` / `12b`、既定 `e4b`、AMD は常に `e4b`）。NSIS の `%LOCALAPPDATA%\{id}` 一括削除対象。
- **フェイルセーフ**: 12B 選択でも本体 GGUF 未取得なら `resolve_effective_proofread_tier` が E4b へフォールバックし、サーバは必ず E4B で起動する。本体と MTP は常に同一階層で揃える。
- モデル階層を変更すると、起動済みの校正エンジン（同梱 llama-server）は旧モデルを保持しているため停止し、次回の AI 校正で新モデルを `-m` 指定で再ロードする。
- 関連: `get_default_llm_model_path` / `get_default_llm_mtp_model_path`（実効階層を解決）、`check_gemma_12b_installed`、`download_gemma_12b`。
...
- 実際に MTP を使うのは **NVIDIA の同梱 llama-server 直起動経路のみ**（`try_start_llama_server_cuda`）。E4B・12B の両階層で同じ配線を使い、`--spec-type draft-mtp` / `--spec-draft-model` / `--spec-draft-n-max 3` / `--spec-draft-ngl 99` を渡す。**FlashAttention は MTP 配線時は `off`、MTP 非併用時のみ `on`**（`flash_attn = if mtp_model_path.is_some() { "off" } else { "on" }`）。MTP ドラフト併用時に `--flash-attn on`（および `auto`）を渡すと、一部 GPU/ビルドで CUDA FlashAttention カーネル（`ggml-cuda/fattn.cu:110`）が致命的に落ち、サーバがポートを開く前にクラッシュする（RTX 4060 Laptop + 同梱 llama.cpp build 9571 で確認。症状は既定 E4B 経路の校正が「AI校正エンジンに接続できませんでした」になる）。MTP の投機的デコード自体は維持する
- **AMD/Lemonade 経路（`try_start_lemonade_bin`）には MTP を配線していない**。`mtp_model_path` は同梱 llama-server（`find_bundled_llama_server_bin`）が存在する時だけ有効で、AMD では None。よって AMD では MTP gguf はダウンロードされても**現状未使用**（llama.cpp 側が非対応なのではなく、アプリが Lemonade 経路へ配線していないため）
- Lemonade の llama.cpp（rocm `b9586` など）自体は `--spec-type` に `draft-mtp` を含み MTP 対応済み。AMD で有効化するには `config.json` の `llamacpp.rocm_args` 等へドラフト指定を注入する実装が要るが、Lemonade のモデル管理（リクエスト単位ロード）との整合は要検証。別タスク扱い
...
- 高精度階層（Gemma 4 12B）は CUDA 版限定・オプトインの追加機能。E4B 既定の挙動を変えないこと、AMD 版へ波及させないこと
