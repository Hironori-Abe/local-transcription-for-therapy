...
EDITOR_TAURI_CONFIG="${LOTT_TAURI_EDITOR_CONFIG:-tauri.editor.linux.override.json}"
EDITOR_TAURI_DEV_CONFIG="${LOTT_TAURI_EDITOR_DEV_CONFIG:-tauri.editor.dev.linux.override.json}"
...
if frontend_ready; then
  ...
  npm --prefix frontend run start -- \
    --host "$FRONTEND_HOST" \
    --port "$FRONTEND_PORT" \
    --build-target lott:build:development,editor &
  FRONTEND_PID="$!"
  FRONTEND_STARTED=1
